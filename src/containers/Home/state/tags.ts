...
export const fetchAllError = (error: any): FetchAllErrorAction => ({
  error,
  type: ActionType.FETCH_ALL_ERROR
});
export const loadAll = (tags: string[]): LoadAllAction => ({
  tags,
  type: ActionType.LOAD_ALL
});
...
export const fetchAll$ = (tags$: Observable<string[]>) => (
  actions$: Observable<Action>
): Observable<Action> =>
  actions$.pipe(
    first(({ type }) => type === ActionType.FETCH_ALL),
    switchMapTo(tags$),
    map(loadAll)
  );
