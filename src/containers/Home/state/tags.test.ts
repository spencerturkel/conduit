...
const testStates = (transform: (initial: State | undefined) => any) => {
  ...
  expect(
    transform({ loading: true, tags: ["hello", "world"] })
  ).toMatchSnapshot();

  expect(
    transform({ loading: false, tags: ["hello", "world"] })
  ).toMatchSnapshot();

  expect(
    transform({ error: "some error", loading: false, tags: ["hello", "world"] })
  ).toMatchSnapshot();
};
...
describe("epics", () => {
  ...
  describe(fetchAll$.name, () => {
    it("should emit LOAD_ALL actions after the first FETCH_ALL action", () => {
      const tags$ = scheduler.createColdObservable<string[]>("--a--b|", {
        a: [],
        b: ["hello", "world"]
      });
      ...
      const actions$ = scheduler.createColdObservable<Action>("-a--b", {
        a: { type: "unknown" },
        b: fetchAll()
      });
    ...
    it("should catch and report errors in the tag source without completing", () => {
      const tags$ = scheduler.createColdObservable<string[]>(
        "#",
        undefined,
        "err"
      );
      ...
      const actions$ = scheduler.createColdObservable<Action>("ab", {
        a: fetchAll(),
        b: fetchAll()
      });
