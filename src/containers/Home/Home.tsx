...
import ArticleListing, {
  ArticlePreview
} from "../../components/Home/ArticleListing";
...
const asyncReducer: AsyncReducer = {
  name: "Home",
...
const AppArticleListing = (() => {
  ...
  const AppFeedPicker = () => (
    <FeedPicker activeFeed={"global"} loggedIn={false} />
  );

  const Result = ArticleListing(
    AppFeedPicker,
    ReactRouterLink as ComponentClass<LinkProps>
  );

  return () => (
    <Result
      previews={articlePreviews}
      onFavoriteCountClick={slug => alert(`Favorited slug ${slug}`)}
    />
  );
})();
...
const AppPopularTags = (() => {
  const Result = connect(
    createSelector(
      selectState,
      createSelector(tags.selectTags, allTags => ({ tags: allTags }))
    )
  )(PopularTags());
