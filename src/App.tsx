...
const rootEpic: Epic<any, any, any> = (
  action$: ActionsObservable<any>,
  middlewareAPI: MiddlewareAPI<any>
) =>
  epic$.pipe(
    mergeMap((epic: Epic<any, any, any>) =>
      epic(action$, middlewareAPI, undefined)
    )
  );
...
const ConnectedAsyncComponent = (
  importFn: () => Promise<{
    default: (props: ContainerProps) => ComponentType<{}>;
  }>
) => {
  const Loader = AsyncComponent(() =>
    importFn().then(({ default: makeComponent }) => ({
      default: makeComponent(componentProps)
    }))
  );
...
const App = () => (
  <BrowserRouter basename="/conduit/">
